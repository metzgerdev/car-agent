...
def test_p4_t7_browser_demo_shell_and_assets_are_served() -> None:
    ...
    assert all("mock listings" not in javascript for javascript in javascript_assets)
    assert all("Six curated listings use linked model-reference photos" not in javascript for javascript in javascript_assets)
...
def test_p4_t13_browser_shows_thinking_placeholder_during_processing() -> None:
    ...
    assert "function UserMessage()" in thread_text
    assert "function ThinkingPlaceholder" in thread_text
    assert "message.status?.type === \"running\" && message.parts.length === 0" in thread_text
