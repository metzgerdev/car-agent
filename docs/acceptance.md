...
| 2026-09-08 | Phase 3 inventory-browse grounding | Explicit requests such as `Show me classic BMWs` route through deterministic `search_inventory` in both offline and live facades, return completed BMW options, and pass 90 Python tests | `dd774fa` | none |
...
| 2026-09-08 | Phase 3 bare model-token grounding | Unique shorthand model references such as `911` now resolve to the matching inventory vehicle and emit `search_inventory → get_vehicle` in deterministic and live facades; 85 Python tests pass | `1fa2688` | none |
...
| 2026-09-08 | Phase 3 service-history trace safety | Live service-history requests now use deterministic `retrieve_service_history` routing, tolerate common maintenance misspellings, ask for a vehicle when context is absent, and cannot claim records without the tool; 88 Python tests pass | `b546ab2` | none |
...
| P3-T14 | Ask for a unique bare model token | A shorthand such as `911` resolves to the unique `1999 Porsche 911 Carrera` inventory record and emits `search_inventory → get_vehicle` in offline and live facades | implemented |
...
| P3-T16 | Ask for maintenance records in live mode | A service-history request for a selected vehicle, including a common typo such as `maintanece records`, routes through `retrieve_service_history` and cannot produce an ungrounded records claim | implemented |
| P3-T17 | Browse inventory by make or category | A request such as `Show me classic BMWs` emits `search_inventory` with a normalized make query and returns completed, grounded inventory options in offline and live facades | implemented |
| P3-T18 | Parse an ambiguous inventory request with an LLM | A request such as `I’m hunting for a classic Beemer for weekends` produces validated `search_inventory` intent data, shows parser confidence in the trace, and executes the existing deterministic search; low-confidence or failed parsing falls back to live conversation | implemented |
