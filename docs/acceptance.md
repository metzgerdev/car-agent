...
| 3 — Sales behavior | complete | CrewAI/OpenRouter foundation, persistent qualification, deterministic ranking, exact availability lookup, explicit deterministic-router grounding and safety routes, unique bare model-token grounding, first-turn explicit vehicle-detail grounding, guaranteed service-history routing with typo tolerance, context-aware vehicle follow-ups, explicit inventory-browse grounding for make/category requests, bounded hybrid LLM context, shared consultative salesperson persona, synthetic service-history retrieval, ambiguity/uncertainty handling, grounded objections, and 91 Python tests pass | Phase 4 API and conversion tests |
