...
| 2026-09-07 | Phase 4 fixed chat composer and auto-scroll | Constrained the conversation to a fixed-height flex surface, kept the composer sticky at the bottom, and explicitly enabled assistant-ui bottom-following for new messages; 77 Python tests, 6 frontend tests, and frontend build pass | pending | none |
...
| P4-T33 | Continue a long chat conversation | The composer remains fixed at the bottom of the conversation surface while the message viewport scrolls internally and follows new messages when the shopper remains at the bottom | implemented |
