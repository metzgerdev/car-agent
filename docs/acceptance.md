...
| P4-T15 | Use the browser voice layer | The microphone requests a server-minted ElevenLabs Scribe token, committed transcripts remain in the composer until the shopper clicks the explicit send button, the message uses the existing `modality: "voice"` chat path, assistant answers are sent to the server-side ElevenLabs speech stream, and missing credentials fail cleanly without exposing the API key | implemented |
