...
| 4 — Conversion and polish | complete | Typed API contract, invalid-schedule protection, idempotent booking, redacted public responses, text/voice command normalization, clean offline demo, assistant-ui styled dark browser UI, streamed tool-trace progress, complete multi-turn tool-trace history, streamed deterministic and live CrewAI answer generation, processing-state Thinking placeholder, curated magazine-review modal, suggested-vehicle review context pass, ElevenLabs Scribe/TTS voice layer, toggleable agent evaluation cockpit, and deterministic side-effect routing for test-drive scheduling; 82 Python tests, 4 frontend trace-history tests, and frontend build pass | none |
...
| 2026-09-06 | Phase 4 ElevenLabs voice layer | FastAPI mints server-side single-use Scribe tokens and streams ElevenLabs speech audio without exposing the API key; the browser captures committed microphone transcripts with `@elevenlabs/react`, sends them through the existing voice-modality `/chat` boundary, and plays assistant answers; 78 Python tests, 4 frontend tests, and frontend build pass | `bf1c48c` | optional incremental audio playback |
| 2026-09-06 | Phase 4 manual voice composer handoff | A committed Scribe transcript remains visible in the live assistant-ui composer, the status prompts `Transcript ready — click send`, and the explicit send button submits it through the existing `/chat` boundary; 78 Python tests, 4 frontend tests, and frontend build pass | `4d12615` | validate autosend separately |
| 2026-09-06 | Phase 4 committed transcript status visibility | The committed-transcript status takes priority over the active Scribe connection state, so the browser shows `Transcript ready — click send` instead of remaining on `Listening…`; 78 Python tests, 4 frontend tests, and frontend build pass | `28b009d` | none |
| 2026-09-06 | Phase 4 manual Scribe commit control | While Scribe is connected, `Commit transcript` calls the provider's explicit commit operation; the transcript then remains in the composer for the separate agent-send click; 78 Python tests, 4 frontend tests, and frontend build pass | `653f045` | validate autosend separately |
| 2026-09-06 | Phase 4 agent evaluation mode | An opt-in evaluation request returns structured route, phase latency, tool, source, confidence, and recommendation-delta metadata over both JSON and SSE; the browser evaluation cockpit renders it without exposing private chain-of-thought, while the right rail remains dedicated to Tool Trace; 80 Python tests, 4 frontend tests, and frontend build pass | `6b95c7f` | none |
...
| P4-T15 | Use the browser voice layer | The microphone requests a server-minted ElevenLabs Scribe token, the shopper can explicitly commit the current partial transcript with `Commit transcript`, the committed text remains in the composer until the separate send button is clicked, the message uses the existing `modality: "voice"` chat path, assistant answers are sent to the server-side ElevenLabs speech stream, and missing credentials fail cleanly without exposing the API key | implemented |
| P4-T16 | Inspect agent evaluation mode | The opt-in mode reports deterministic versus CrewAI/OpenRouter routing, phase latency, tool counts, source coverage, qualitative confidence, and before/after recommendation changes over JSON and SSE; the browser renders an evaluation cockpit while normal turns remain unchanged | implemented |
| P4-T17 | Arrange a test drive in live mode | Scheduling uses the deterministic side-effect safety route, validates required details, emits `schedule_test_drive` in the streamed Tool Trace, and never confirms a booking without a successful tool result | implemented |
