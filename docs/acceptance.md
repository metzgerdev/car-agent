...
| 3 — Sales behavior | complete | CrewAI/OpenRouter foundation, persistent qualification, deterministic ranking, REST-style resource tool semantics, exact availability lookup, first-turn comparison grounding, explicit deterministic-router grounding and safety routes, unique bare model-token grounding, first-turn explicit vehicle-detail grounding, guaranteed service-history routing with typo tolerance, context-aware vehicle follow-ups, explicit inventory-browse grounding for make/category requests, validated LLM intent parsing for ambiguous read-only searches, hydrated recommendation listings, bounded hybrid LLM context, shared consultative salesperson persona, synthetic service-history retrieval, ambiguity/uncertainty handling, grounded objections, and 96 Python tests pass | Phase 4 API and conversion tests |
...
| 2026-09-08 | Phase 3 inventory-browse grounding | Explicit requests such as `Show me classic BMWs` route through deterministic `list_inventory` in both offline and live facades, return completed BMW options, and pass 90 Python tests | `dd774fa` | none |
| 2026-09-08 | Phase 3 browse-phrase regression | Category phrasing such as `Find a weekend sports car` also completes a deterministic inventory search, while review/service-history phrases remain excluded; 91 Python tests pass | `c90d273` | none |
| 2026-09-08 | Phase 3 hybrid intent routing | Ambiguous read-only requests are classified into a Pydantic intent envelope, emitted in the trace with confidence, and routed to deterministic inventory search only above the confidence threshold; parser failures fall back to CrewAI; 93 Python tests pass | `1df49d0` | none |
| 2026-09-08 | Phase 3 hydrated recommendation listings | The first two ranked search results are loaded through `get_vehicle`, merged into the recommendation records, and rendered with grounded specifications; 93 Python tests pass | `771e647` | none |
| 2026-09-08 | Phase 3 REST-style tool semantics | Inventory collection, vehicle resource, vehicle subresources, comparison resource, and test-drive creation tools use consistent resource-oriented names and trace contracts; 93 Python tests pass | `c98d3d7` | none |
| 2026-09-08 | Phase 3 first-turn comparison grounding | Explicit comparisons route through deterministic `get_vehicle_comparison` before CrewAI, so omitted years are not invented and the response uses the two named inventory records; 94 Python tests pass | `4ed1c08` | none |
| 2026-09-08 | Phase 3 affirmative contextual follow-up | Review responses persist an explicit `service_history` follow-up and a bare affirmative reply routes to `get_service_history` for the active vehicle exactly once in deterministic and live facades; 96 Python tests pass | `6794bcb` | none |
| 2026-09-08 | Phase 4 varied starter prompts | Added six additional enabled starter prompts covering facts, magazine reviews, service history, comparison, exact availability, and vehicle details; frontend tests and production build pass | `ab9615d` | none |
...
| 2026-09-05 | Phase 3 service-history retrieval | Mock inventory records carry typed synthetic `ServiceRecord` entries, SQLite normalization preserves them, and both deterministic and CrewAI paths expose `get_service_history`; 73 Python tests plus frontend build pass | `e5767c3` | none |
...
| 2026-09-06 | Phase 4 test-drive trace safety | Live-mode scheduling now routes through deterministic validation, so a successful confirmation is always paired with a `create_test_drive` tool event in the SSE stream; invalid or incomplete requests remain unbooked; 82 Python tests pass | `aae0a97` | none |
...
| 2026-09-07 | Phase 4 press-review intent regression | Added `press review(s)`, automotive press, car reviews, and auto reviews to the deterministic review-intent route; the reproduced Z4 follow-up calls `get_magazine_reviews` and returns its citation; 74 Python tests and the 20-scenario evaluator pass | `f30e7b9` | none |
...
| 2026-09-08 | Phase 3 bare model-token grounding | Unique shorthand model references such as `911` now resolve to the matching inventory vehicle and emit `list_inventory → get_vehicle` in deterministic and live facades; 85 Python tests pass | `1fa2688` | none |
| 2026-09-08 | Phase 3 first-turn vehicle-detail grounding | Explicit requests such as `Tell me about the Honda S2000` now route through deterministic `get_vehicle` grounding in live mode, ensuring the SSE Tool Trace includes the lookup before the answer; 86 Python tests pass | `8fc6f31` | none |
| 2026-09-08 | Phase 3 service-history trace safety | Live service-history requests now use deterministic `get_service_history` routing, tolerate common maintenance misspellings, ask for a vehicle when context is absent, and cannot claim records without the tool; 88 Python tests pass | `b546ab2` | none |
...
| P1-T2 | Add use and driving style | Agent emits the ordered trace `list_inventory → get_vehicle × 2 → get_vehicle_facts` | implemented |
...
| ID | Scenario | Expected result | Status |
| --- | --- | --- | --- |
| P3-T0 | Construct the CrewAI sales crew and OpenRouter configuration | One CrewAI agent, one structured task, all domain tools including `get_service_history`, and an OpenRouter LLM configuration are available without exposing the key | implemented |
...
| P3-T12 | Ask for listing service history | Typed synthetic service records are returned by the dedicated `get_service_history` tool, appear in the trace, are disclosed as synthetic, and do not replace general ownership facts | implemented |
...
| P3-T14 | Ask for a unique bare model token | A shorthand such as `911` resolves to the unique `1999 Porsche 911 Carrera` inventory record and emits `list_inventory → get_vehicle` in offline and live facades | implemented |
| P3-T15 | Ask about an explicitly named vehicle on the first turn | A request such as `Tell me about the Honda S2000` resolves the single matching listing through `get_vehicle` in live mode instead of allowing an ungrounded model response | implemented |
| P3-T16 | Ask for maintenance records in live mode | A service-history request for a selected vehicle, including a common typo such as `maintanece records`, routes through `get_service_history` and cannot produce an ungrounded records claim | implemented |
| P3-T17 | Browse inventory by make or category | A request such as `Show me classic BMWs` emits `list_inventory` with a normalized make query and returns completed, grounded inventory options in offline and live facades | implemented |
| P3-T18 | Parse an ambiguous inventory request with an LLM | A request such as `I’m hunting for a classic Beemer for weekends` produces validated `list_inventory` intent data, shows parser confidence in the trace, and executes the existing deterministic search; low-confidence or failed parsing falls back to live conversation | implemented |
| P3-T19 | Enrich ranked inventory recommendations | The first two search results are hydrated through `get_vehicle` and their grounded body style, transmission, drivetrain, and horsepower are included in the recommendation response | implemented |
| P3-T20 | Inspect resource-oriented tool contracts | CrewAI exposes `list_inventory`, `get_vehicle`, `get_vehicle_facts`, `get_service_history`, `get_magazine_reviews`, `get_vehicle_comparison`, and `create_test_drive`; exact identity lookup remains an explicit match-status contract | implemented |
| P3-T21 | Compare two named models on the first turn | A request such as `Compare the Honda S2000 and Porsche 911 Carrera` emits `get_vehicle_comparison` and returns the actual 2004 and 1999 inventory records without fabricating a shared year | implemented |
| P3-T22 | Confirm an offered review follow-up | After a review response offers maintenance history, a reply such as `yes` resolves the active vehicle, emits `get_service_history`, returns the records, and does not repeat the offer in offline or live-facade mode | implemented |
...
| P4-T16 | Arrange a test drive in live mode | Scheduling uses the deterministic side-effect safety route, validates required details, emits `create_test_drive` in the streamed Tool Trace, and never confirms a booking without a successful tool result | implemented |
...
| P4-T36 | Demonstrate varied starter-prompt routes | The empty chat exposes nine clickable prompts covering inventory listing, vehicle detail, facts, magazine reviews, service history, comparison, exact availability, and another grounded vehicle lookup | implemented |
