...
| 3 — Sales behavior | complete | CrewAI/OpenRouter foundation, persistent qualification, deterministic ranking, exact availability lookup, explicit deterministic-router grounding and safety routes, unique bare model-token grounding, first-turn explicit vehicle-detail grounding, guaranteed service-history routing with typo tolerance, context-aware vehicle follow-ups, bounded hybrid LLM context, shared consultative salesperson persona, synthetic service-history retrieval, ambiguity/uncertainty handling, grounded objections, and 20-scenario evaluation pass; 88 Python tests pass | Phase 4 API and conversion tests |
