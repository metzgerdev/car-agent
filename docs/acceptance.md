...
| 2026-09-06 | Phase 4 voice autosend regression | A committed Scribe transcript is written to the live assistant-ui composer and sent through the composer client once it is sendable; the served bundle and source contract cover the regression; 78 Python tests, 4 frontend tests, and frontend build pass | `cdee70c` | none |
...
| P4-T15 | Use the browser voice layer | The microphone requests a server-minted ElevenLabs Scribe token, committed transcripts use the existing `modality: "voice"` chat path, assistant answers are sent to the server-side ElevenLabs speech stream, and missing credentials fail cleanly without exposing the API key | implemented |
