...
| 2026-09-11 | Inventory-backed reference resolution | Follow-up references resolve through canonical inventory IDs, scoped to the listings already shown; explicit full identities override context, and focused listings are retained separately from result sets. Recommendation traces now avoid redundant vehicle hydration. | pending | none |
...
| P1-T2 | Add use and driving style | Agent emits the ordered trace `list_inventory → get_vehicle_facts` | implemented |
...
| P3-T19 | Enrich ranked inventory recommendations | `list_inventory` provides the grounded listing details and the top match receives a sourced ownership fact without redundant `get_vehicle` calls | implemented |
