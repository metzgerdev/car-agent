...
listing preview with model-reference photos for the six curated vehicles and
key vehicle specs. The source page, photographer, and license are shown in the
preview; generated listings without a mapped photo use a labeled CSS fallback.
...
The checked-in inventory contains 100 deterministic mock records: six
curated vehicles used by the scripted demo plus 94 auction-style records
...
`synthetic_demo`. The six curated gallery vehicles include real model-reference
photos from Wikimedia Commons with photographer, source-page, and license
credits; generated records use a labeled fallback when no photo is mapped.
